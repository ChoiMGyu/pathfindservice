/*
 * 클래스 기능 : 이메일 유효성 검증 및 존재 여부를 반환하는데 쓰이는 dto 클래스
...
@Schema(name = "EmailExistVCResponse", description = "유저 이메일 유효성 검증 통과 여부 반환에 쓰이는 dto")
@Data
@AllArgsConstructor
public class EmailExistVCResponse {
    /*@Schema(name = "EmailCheck", description = "유저 이메일 유효성 검증 여부", example = "true")
    private boolean EmailCheck; // 이메일 유효성 검증 여부*/
    /*@Schema(name = "isSame", description = "존재 여부 확인", example = "true")
