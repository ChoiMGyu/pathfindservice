...
public class AccountRecoveryApiController {
    ...
    // 아이디 찾기 시 이메일 존재 여부를 검사하는 함수이다.
    @Operation(summary = "이메일 존재 여부 확인", description = "아이디 찾기 시 이메일 존재 여부를 확인하고 그 결과를 반환합니다.")
    @ApiErrorCode(EmailExistErrorCode.class)
    @GetMapping("/exist/email")
    public EmailExistVCResponse isValidEmail(@Parameter(name = "email", description = "문자열 형태의 이메일") @ModelAttribute(name = "email") @Validated(ValidationSequence.class) EmailExistVCRequest form) {
    ...
    @Operation(summary = "이메일, 인증 번호 일치 여부 확인, 아이디 반환", description = "아이디 찾기 시 이메일과 인증번호 존재 여부를 확인하고 아이디를 반환합니다.")
    ...
    @GetMapping("/userid")
    public FindUserIdVCResponse findUserId(@ModelAttribute(name = "EmailChkVCRequest") @Validated(ValidationSequence.class) FindUserIdVCRequest form) {
    ...
    // 비밀번호 초기화 시 아이디와 이메일 정보가 일치하는지 여부를 검사하는 함수이다.
    @Operation(summary = "아이디, 이메일 일치 여부 확인", description = "비밀번호 초기화 시 아이디, 이메일 일치 여부를 확인하고 그 결과를 반환합니다.")
    ...
    public IdEmailExistVCResponse isValidIdEmail(@ModelAttribute(name = "IdEmailExistVCRequest") @Validated(ValidationSequence.class) IdEmailExistVCRequest form) {
    ...
    // 비밀번호 찾기 시 아이디, 이메일, 이메일 인증 번호의 일치 여부를 확인하고 회원의 비밀번호를 초기화한 후 초기화된 비밀번호를 이메일로 전송하는 함수이다.
    @Operation(summary = "아이디, 이메일, 이메일 인증 번호 일치 여부 확인, 비밀번호 초기화", description = "아이디, 이메일, 이메일 인증 번호 일치 여부를 확인하고 비밀번호를 초기화한 후 그 결과를 반환합니다.")
    ...
    public PasswordResetVCResponse resetPassword(@RequestBody @Validated(ValidationSequence.class) PasswordResetVCRequest form) {
